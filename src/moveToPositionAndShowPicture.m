function data = moveToPositionAndShowPicture(x, y, framesCount)
    % Moves camera to specified position and returns the picture(s).
    % Data is either single image (in case of framesCount being 1)
    % or array of images. In the latter case indexing goes as follows:
    % (imageNo, X, Y, RGB).
    % When omitted, x and y default to the center position for "Komar 2",
    % framesCount defaults to 1.
